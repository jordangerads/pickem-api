...
public class PicksServiceImplTest {
    ...
    private void setupGamesMocks() {
        ...
        when(gameRepository.findAllByWeek(anyInt())).thenAnswer(invocation -> gameMap.values().stream().filter(game -> game.getWeek().equals(invocation.getArgumentAt(0, Integer.class))).collect(Collectors.toList()));
    }
